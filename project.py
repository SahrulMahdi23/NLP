...
kata_kunci_positif = [
    ...
    "recommended", "cepet", "bgt", "banget" # 'bgt'/'banget' sering muncul dengan kata positif
]
kata_kunci_negatif = [
...
for ulasan in df['Review']:
    ulasan_lower = str(ulasan).lower() # Ubah ke huruf kecil dan string
    ...
    if ada_positif and not ada_negatif:
            ...
            label_sentimen_berdasarkan_aturan.append('positif') # Default jika tidak ada keyword jelas
...
def _internal_preprocess_text(text):
  text = str(text).lower()
  text = re.sub(r'[^a-z\s]', '', text)# Hanya mempertahankan huruf dan spasi
...
if len(y) == 0:
    ...
    exit()
elif len(y) < 2 : # atau jumlah lain yang terlalu kecil untuk dibagi
...
model = DecisionTreeClassifier(random_state=42, max_depth=10) # Batasi kedalaman untuk visualisasi
